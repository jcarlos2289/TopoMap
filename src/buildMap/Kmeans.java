...
public class Kmeans  {
	...
	// Constructor
    public Kmeans (int ki, int dimi, ArrayList<ImageTags> ImageList) {  //recibiria arraylist de imagetags
    	if (ki>0 && dimi>0) {			//saco el keyset con el orden
	        ...
	        for (int i=0; i<k; i++) 
	        	means[i]=new Point(dim);  //enviar el entryset + los datos[hashmap]
		}									//almacenar todo el arraylist<ImageTag> como arrarylist>point> obtanied
    ...
    private void inicializeMeans () {
    	for (int i=0; i<k; i++) {
            means[i].copy(obtained.get((int)(Math.random()*obtained.size())));
        	nPointsMean[i]=0;
    	}
    }
    ...
    private boolean change (Point []means, Point []meansAux) {
    	...
    	for (int p=0; p<k; p++)
		    for (int i=0; i<dim; i++) 
	    		desv += (means[p].get(i)-meansAux[p].get(i))*(means[p].get(i)-meansAux[p].get(i));
    ...
    // In this method we have to ensure that every cluster has at least one Point
    private void ensureNotEmpty () {
    	int acum[]=new int[k];
    	double d;
    	int ind;

    	for (int m=0; m<k; m++) acum[m]=0;
    	...
    	for (int m=0; m<k; m++)
    		if (acum[m]==0) {
    			d=dist.get(0);
    			ind=0;
    			// The cluster is empty. We put a Point which has the big distance to its mean
